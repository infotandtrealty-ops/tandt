...
const Header = () => {
  ...
  const serviceCategories = {
    commercial: [
      { label: "SCO Plots", link: "/services" },
      { label: "Retail Shops", link: "/services" },
      { label: "Office Spaces", link: "/services" }
    ],
    residential: [
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        ...
        {Object.keys(megaMenuContent).map((key) => (
          <div key={key} className="border-b border-white/20 pb-2">
            ...
            >
              {key === "about" ? "What We Do" : key === "services" ? "What We Are" : key.replace("-", " ")} <ChevronDown className={cn("w-5 h-5 transition-transform text-white", mobileDropdown === key && "rotate-180")} />
            </button>
            <AnimatePresence>
              {mobileDropdown === key && (
...
<Button
  ...
  className="w-full bg-gold hover:bg-gold/90 text-[#031916] h-14 rounded-xl text-lg font-bold"
>
  ENQUIRE NOW
</Button>
      </div>
